        ...
        <div className="flex p-10  max-md:p-5 bg-main-gray rounded-lg gap-5 flex-col">
            ...
            {data.map((data,index)=>(
                <Link href={`/request/${data.id}/${data.currencyReceived}`} className=" border-b-2 gap-1 max-md:grid-cols-1 pb-5 grid grid-cols-5" key={data.email+ index + data.id}>
                    ...
                    <p className=" flex justify-between"><span className=" md:hidden">Отдаете</span>{data.amountSent+" "+data.currencyReceived}</p>
                    <p className=" flex justify-between"><span className=" md:hidden">Получаете</span>{data.amountReceived+" "+data.currencySent}</p>
                    <p className={clsx(
